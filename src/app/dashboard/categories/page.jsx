...
function Categories({ swal }) {
  ...
  const protectedWay = session?.user?.email === "johndoe@gmail.com"
    ...
    <section className="min-h-screen">
      ...
      <form onSubmit={saveCategory} className="flex gap-2 w-full">
        <div className="flex flex-col gap-2">
            ...
            >
              ...
              {categories.length > 0 &&
                categories.map((category, index) => (
                  <option value={category._id} key={index} className="w-1/2">
                    {category.name}
                  </option>
                ))}
            </select>
          ...
          <button className="btn-default" onClick={addPropreties} type="button">
            Add propreties
          </button>
          {propreties.length > 0 &&
            propreties.map((el, index) => (
              <div className="flex gap-1 flex-wrap md:flex-nowrap" key={index}>
                <input
                  ...
                  value={el.name}
                  onChange={(ev) =>
                    handlePropretyNameChange(index, el, ev.target.value)
                  }
                />
                <input
                  type="text"
                  ...
                  placeholder="Value"
                  onChange={(ev) =>
                    handlePropretyValueChange(index, el, ev.target.value)
                  }
                />
                <button
      ...
      {!editedCategory && (
        <table className="min-w-full md:min-w-0 mt-4 bg-blue-200">
          <thead>
            <tr>
              <td className="w-1/3 md:w-full">Category name</td>
            ...
            {categories.length > 0 &&
              categories.map((category, index) => (
                <tr key={index} className="bg-white">
                  <td className="pl-2 w-1/3 md:w-full">{category.name}</td>
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger>
                            <Button
                              className={clsx(
                                "btn-primary hover:bg-inherit text-secondary font-semibold"
                              )}
                              disabled={protectedWay}
                            >
                              <svg
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger>
                            <Button
                              className={clsx(
                                "btn-secondary hover:bg-inherit text-secondary font-semibold"
                              )}
                              disabled={protectedWay}
                            >
                              <svg
