...
export default function NavFront() {
    ...
    return (
        <>
    <header className="text-white flex bg-base-100 px-10 justify-between relative bg-slate-700 relative">
        ...
        <button
            ...
            className="order-3 lg:order-2 lg:hidden gap-4 items-center justify-end flex text-black [&>svg]:fill-white [&>svg]:text-white">
        ...
        <nav
            ...
            <ul className="items-start list-nav whitespace-nowrap px-1 flex flex-col lg:flex-row w-full lg:flex-nowrap lg:w-fit lg:items-center gap-4">
                <li><a href="/homepage" className="link-nav w-full">
                    ...
                    Home</a></li>
                <li><a href="/homepage/products" className="link-nav w-full">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5"
                         stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round"
                ...
                <li tabIndex={0} className="link-nav">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent className="bg-slate-700 border-none">
                            <DropdownMenuItem className="text-white"><a>My account</a></DropdownMenuItem>
                            <DropdownMenuSeparator className="bg-white w-[90%] m-auto"/>
                            <DropdownMenuItem><ButtonLogoutFront/></DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </li>
            </ul>
    ...
    <div onClick={() => setNavActive(true)}
            className={`absolute w-full h-full bg-slate-500 z-10 ${!navActive ? 'block ' : 'hidden'} opacity-40`}></div>
        </>
