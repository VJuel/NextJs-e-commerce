...
export default function FormPayement({loading}) {
    ...
    function checkForm(data) {
        const {name,email,city,codePostal,streetAdress,country} = data
            ...
            const {success, error: zodError} = PayementSchema.safeParse({name,email,city,codePostal,streetAdress,country})
            if (!success) {
     ...
     async function goToPayment() {
        const data = {name,email,city,codePostal,streetAdress,country,cartProducts}
    ...
    return (
        <form method="post" 
                    ...
                    {validationError?.city && <p className={errorForm}>{validationError.city._errors.join(', ')}</p>}
                    {validationError?.codePostal && <p className={errorForm}>{validationError.codePostal._errors.join(', ')}</p>}
                    ...
                    {validationError?.streetAdress && <p className={errorForm}>{validationError.streetAdress._errors.join(', ')}</p>}

                    <input type="text" placeholder="country" name="country" value={country} onChange={(ev) => setCountry(ev.target.value)}/>
                    {validationError?.country && <p className={errorForm}>{validationError.country._errors.join(', ')}</p>}

                    <input type="hidden" name="products" value={cartProducts.join(',')}/>
                    {validationError?.cartProducts && <p className={errorForm}>Error avec le panier</p>}
